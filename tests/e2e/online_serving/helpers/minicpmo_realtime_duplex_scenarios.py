...
class DemoState:
    ...
    def response_transcript_delta(self, response_id: str) -> str:
        return "".join(
            str(event.get("delta", ""))
            for event in self.events
            if event.get("type") == "response.output_audio_transcript.delta" and self._event_response_id(event) == response_id
        )
    ...
    def response_transcript_done(self, response_id: str) -> list[str]:
        return [
            ...
            for event in self.events
            if event.get("type") == "response.output_audio_transcript.done" and self._event_response_id(event) == response_id
        ]
...
async def run_demo(args: DemoArgs) -> dict[str, object]:
    ...
    realtime_audio_lifecycle_ok = state.count("response.output_audio.delta") > 0 and state.count("response.output_audio.done") > 0
