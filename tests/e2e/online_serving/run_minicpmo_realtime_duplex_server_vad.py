...
async def run_server_vad_interrupt(args) -> dict[str, object]:
    ...
    try:
        ...
        assert target_id
        await until(lambda: _events(client.events.events, target_id, "response.output_audio.delta"), "response.output_audio.delta")
    ...
    stale = any(
        e.get("type") == "response.output_audio.delta" and RealtimeEventCollector.response_id(e) == target_id for e in trailing
    )
    followup_audio = any(
