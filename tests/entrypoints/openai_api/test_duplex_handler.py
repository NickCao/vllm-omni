...
def test_native_realtime_protocol_audio_delta_preserves_sample_rate_hz():
    ...
    audio_events = [
        payload for payload in payloads if payload["type"] in {"response.output_audio.delta", "response.output_audio.delta"}
    ]
...
def test_native_realtime_protocol_terminal_transcript_equals_joined_deltas():
    ...
    transcript = "".join(str(event["delta"]) for event in events if event["type"] == "response.output_audio_transcript.delta")
