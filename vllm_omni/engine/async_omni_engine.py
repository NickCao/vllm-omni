...
class AsyncOmniEngine:
    ...
    def stage_clients(self) -> list[StageClient]:
        """Logical-stage view for external readers: one client per stage.

        Recomputed from ``self.stage_pools`` on every access rather than
        cached, so a dead replica swapped out for a healthy one via
        ``StagePool.add_client``/``remove_client`` (e.g. after a stage
        restart in distributed mode) is reflected immediately. A stale
        snapshot here previously left ``OmniBase._has_dead_stage`` latched
        on a long-gone client forever.
        """
